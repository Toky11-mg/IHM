...
// ─── Horloge isolée ───────────────────────────────────────────────────────────
function Horloge({ grade, specialite }: { grade?: string; specialite?: string }) {
  ...
  return (
    <div style={{ textAlign: 'right' }}>
      ...
      {grade && specialite && (
        <div style={{ fontSize: '12px', color: '#9ca3af' }}>{grade} · {specialite}</div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '1.5rem' }}>
        ...
        <div style={S.card}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            ...
            <button onClick={() => navigate('/enseignant/mes-matieres')} style={{ fontSize: '12px', color: ENI, background: 'none', border: 'none', cursor: 'pointer', fontWeight: 500 }}>
              Voir tout →
            </button>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {profile.matieres.map(m => (
                <div
                  ...
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', borderRadius: '10px', border: '0.5px solid #e5e7eb', cursor: 'pointer' }}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {QUICK.map(({ label, path, icon, color, bg }) => (
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 14px', borderRadius: '10px', border: '0.5px solid #e5e7eb', backgroundColor: '#fff', cursor: 'pointer', textAlign: 'left' }}
